...
from item_analysis_title import (
    AnalysisTitleType,
    ColAnalysisTitleItem,
    RowAnalysisTitleItem,
)
...
def create_detail(jsoninfo: ConfigureData, helper: XlsHelper):
        ...
        if isinstance(titleitem, TitleItem):
            ...
            if ishave_sectitle:
                ...
                for tmplie in range(secondmenu_len):
                    ...
                    worksheet.write(
                        lineindex + 1, new_col, sectitle_showname, sectitle_format
                    )
            ...
            if is_colsum_dicresult_here:
                colsumdic_callfunc(
                    colsum_dic, colsumdic_value, "set_resultindex", listindex
                )
            listindex = listindex + 1
    ...
    if breeze_listindex != -1:
        worksheet.freeze_panes(
            first_showday_line, breeze_listindex, first_showday_line, 0
        )
    for c_month in range(max_month):
        ...
        if (
            breeze_listindex != -1
            and int(freeze_detail) == c_month
            and jsoninfo.get_isfreezemonth()
        ):
            worksheet.freeze_panes(first_showday_line, breeze_listindex, lineindex, 0)
        for day_item in obj.itermonthdays4(c_year, c_month):
            ...
            if (
                breeze_listindex != -1
                and int(freeze_detail) - 1 == lineindex
                and jsoninfo.get_isfreezeline()
            ):
                worksheet.freeze_panes(
                    first_showday_line, breeze_listindex, lineindex, 0
                )
            # 日期：年月日，显示格式:x月x日
                            ...
                            if bhave_sec:
                                for v_key, v_value in attr_value.items():
                                    for tmpcol in range(maxlist_index):
                                        if (
                                            tmpcol in column_title_dic
                                            and column_title_dic[tmpcol] == v_key
                                        ):
                                            worksheet.write(lineindex, tmpcol, v_value)
                                            dataItem.set_data_property(
                                                name, v_key, v_value
                                            )
                                            break
                                if countcell_num is not None:
                                    coutcell_colindex = (
                                        titleitem.get_countcell_col_index()
                                    )
                                    ...
                                    dataItem.set_data_property_countcell(
                                        name, countcell_num
                                    )
                            else:
                                # 一级列表
                                ...
                                for tmpcol in range(maxlist_index):
                                    if (
                                        tmpcol in column_title_dic
                                        and column_title_dic[tmpcol] == attr
                                    ):
                                        if des is not None:
            ...
            if cn_weeknum_str == "周日":
                ...
                sumstr = get_sumstr(
                    colsum_dic, weekcount_key, week_start_lineindex, week_end_lineindex
                )
                ...
                month_dataitem.add_weekitem_row(
                    week_start_lineindex, week_end_lineindex
                )
            ...
            if curday_index == month_totoal_daynum:
                ...
                if cn_weeknum_str != "周日":
                    ...
                    if isinstance(result, ColSumResult):
                        lineindex = lineindex + 1
                        worksheet.write(
                            lineindex, result.result_index, sumstr, sum_format
                        )
                ...
                if isinstance(result, ColSumResult):
                    ...
                    worksheet.write(
                        lineindex, result.result_index, sumstr, month_end_format
                    )
                new_week = True
    ...
    for m in range(max_month):
        ...
        for col_index in range(col_len):
            ...
            if isinstance(item, ColAnalysisTitleItem):
                ...
                itemcellformat = (
                    helper.get_analysis_itemcell_format(item)
                ...
                for row_index in range(row_len):
                    ...
                    if isinstance(monthdata, AnalysisMonthDataItem):
                        if col_index < col_data_num and row_index < row_data_num:
                            ...
                            worksheet.write(
                                row_cur + row_index + 1, col_need, value, itemcellformat
                            )
                            if row_index == 0:
                                ...
                                chart_name = (
                                    f"={sheetname}!{charttitle_col_start_cellstr}"
                                )
                    ...
                    if row_index == row_len - 1:
                        ...
                        worksheet.write(
                            row_cur + row_index + 1, col_need, sumstr, itemcellformat
                        )
            # endregion
                    ...
                    if is_handle_itemcell:
                        ...
                        for c in range(col_len):
                            if c < col_content - 2:
                                if c == col_content - 3:
                                    worksheet.write(
                                        row_need, c + 1, None, itemcellformat
                                    )
                                else:
                                    cs = ColSumResult()
                                    cs.set_index(
                                        row_need - 1 - 7, row_need - 2, c + 1, c + 1
                                    )
                                    sumstr = cs.get_sumstr(XlsHelper)
                                    worksheet.write(
                                        row_need, c + 1, sumstr, itemcellformat
                                    )
        # endregion
...
def write_chart_item(helper: XlsHelper, month):
    ...
    chart.set_x_axis(
        {
            "num_font": {"color": "#ababb9", "name": "微软雅黑", "size": 9},
            "line": {"none": True},
        }
    )
    chart.set_y_axis(
    ...
    chart.set_legend(
        {
            "position": "bottom",
            "font": {"color": "#ababb9", "name": "微软雅黑", "size": 9},
        }
    )
                    ...
                    for dayrow in row_list:
                        if dayrow in dataItem_dic:
                            ...
                            if isinstance(dataitem, DataItem):
                                ...
                                analy_monthdataitem.set_data(
                                    weekindex, weekdayindex, daycoust
                                )
